...
if (x > obj_knight.x + range/2){
	target_x = obj_knight.x + range/2;
}
else if (x < obj_knight.x - range/2){
	target_x = obj_knight.x - range/2;
}
//if (obj_knight.image_xscale > 0) target_x = obj_knight.x + range/2;
//if (obj_knight.image_xscale < 0) target_x = obj_knight.x - range/2;


//peeking
if (obj_knight.image_xscale > 0){
	if (peek_x < peek) peek_x += (peek - peek_x) * .025;
}
else  if (peek_x > -peek) peek_x -= (peek_x + peek) * .025;

/*if (obj_knight.key_up) peek_up_counter += 1/room_speed;
else peek_up_counter = 0;

if (obj_knight.key_down) peek_down_counter += 1/room_speed;
